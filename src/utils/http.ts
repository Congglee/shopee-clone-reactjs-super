  ...
  constructor() {
    ...
    this.instance = axios.create({
      ...
      headers: {
        ...
        'expire-access-token': 2 * 24 * 60 * 60 * 1000,
        'expire-refresh-token': 7 * 24 * 60 * 60 * 1000
      }
    })
      ...
      this.instance.interceptors.response.use(
        ...
        (error: AxiosError) => {
          ...
          if (isAxiosUnauthorizedError<ErrorResponse<{ name: string; message: string }>>(error)) {
            ...
            // Trường hợp token hết hạn và request đó không phải là của refresh token thì mới tiến hành gọi refresh token
            ...
            // Những trường hợp như token không đúng
            // Không truyền token
